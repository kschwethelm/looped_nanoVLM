...
def compute_ranking_summary(all_results, tasks_to_plot):
    ...
    for step in all_steps:
        ...
        if tasks_to_plot:
            ...
            metrics_to_rank = [
                k for k in step_data[0].keys() if k not in ["step", "average", "average_rank"]
            ]
        ...
        with open(json_file) as f:
            ...
            if tasks_to_plot and "average" in tasks_to_plot:
                ...
                for task in tasks_to_plot:
                    if (
                        task != "average"
                        and "rank" not in task.lower()
                        and task in result
                        and isinstance(result[task], (int, float))
                    ):
...
def plot_results(
    ...
    for metric in individual_plots:
        if metric in metric_names:
            save_individual_plot_pdf(
                all_results, eval_folders, custom_names, output_filename, metric, steps_to_plot
            )
...
def save_individual_plot_pdf(
    all_results, eval_folders, custom_names, output_filename, metric_name, steps_to_plot=None
):
    ...
    for j, (results, eval_folder) in enumerate(zip(all_results, eval_folders)):
        ...
        if values:
            ...
            if stderr_values and any(stderr > 0 for stderr in stderr_values):
                ...
                upper_bounds = [v + s for v, s in zip(values, stderr_values)]
                plt.fill_between(
                    metric_steps, lower_bounds, upper_bounds, color=color, alpha=0.2, linewidth=0
                )
...
def save_csv_data(
    all_results, eval_folders, custom_names, metric_names, output_file, steps_to_plot=None
):
  ...
  python plot_eval_results.py Exp1:/path/to/eval1 Exp2:/path/to/eval2 --tasks mmlu""",
    ...
    parser.add_argument(
        ...
        default=[
            ...
            "scienceqa_exact_match",
            "textvqa_val_exact_match",
            "average",
        ],
        nargs="+",  #'ai2d_exact_match',
    ...
    parser.add_argument(
        "--output", type=str, help="Custom filename for the saved plot (without extension)"
    )
    parser.add_argument(
        "--steps",
        nargs="+",
...
def main():
            ...
            if steps_to_plot:
                ...
                if missing_steps:
                    folder_name = (
                        ...
                        if custom_names and custom_names[eval_folders.index(eval_folder)]
                        else os.path.basename(eval_folder)
                    )
    ...
    if any(all_results):
        ...
        plot_results(
            all_results, eval_folders, custom_names, tasks_to_plot, output_filename, steps_to_plot
        )
    else:
