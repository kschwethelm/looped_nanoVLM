...
def get_run_name(train_cfg, vlm_cfg):
    batch_size = (
        f"bs{int(train_cfg.batch_size * get_world_size() * train_cfg.gradient_accumulation_steps)}"
    )
...
def train(train_cfg, vlm_cfg):
    ...
    if train_cfg.lr_vision_backbone > 0:
        param_groups.append(
            {"params": list(model.vision_encoder.parameters()), "lr": train_cfg.lr_vision_backbone}
        )
    else:
    ...
    if train_cfg.lr_language_backbone > 0:
        param_groups.append(
            {"params": list(model.decoder.parameters()), "lr": train_cfg.lr_language_backbone}
        )
    else:
            ...
            if is_update_step:
                ...
                if train_cfg.lr_vision_backbone > 0:
                    adj_lr_vision_backbone = get_lr(
                        global_step, train_cfg.lr_vision_backbone, train_cfg.max_training_steps
                    )
                ...
                if train_cfg.lr_language_backbone > 0:
                    adj_lr_language_backbone = get_lr(
                        global_step, train_cfg.lr_language_backbone, train_cfg.max_training_steps
                    )
    ...
    if is_master():
        ...
        batch_size = int(
            train_cfg.batch_size * get_world_size() * train_cfg.gradient_accumulation_steps
        )
...
def main():
    ...
    parser.add_argument(
        "--lr_language_backbone", type=float, help="Learning rate for the language backbone"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--relevance_min_rating", type=int, help="Minimum relevance rating of images per sample"
    )
    parser.add_argument(
