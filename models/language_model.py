...
class LanguageModelGroupedQueryAttention(nn.Module):
    ...
    def forward(
        ...
        k_curr = (
            self.k_proj(x).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2)
        )  # (B, n_kv_heads, T_curr, head_dim)
        v_curr = (
            self.v_proj(x).view(B, T_curr, self.n_kv_heads, self.head_dim).transpose(1, 2)
        )  # (B, n_kv_heads, T_curr, head_dim)
...
class LanguageModel(nn.Module):
    ...
    @staticmethod
    def _copy_block_weights(state_dict: dict, src_prefix: str, dst_prefix: str):
        """
        Helper function to copy weights from one block to another.

        Args:
            state_dict: The state dictionary to modify
            src_prefix: Source prefix (e.g., 'blocks.0.')
            dst_prefix: Destination prefix (e.g., 'prelude_blocks.0.')
        """
        # Define all the keys within a block
        block_keys = [
            "attn.q_proj.weight",
            "attn.k_proj.weight",
            "attn.v_proj.weight",
        ...
        for key in block_keys:
            ...
            if src_key in state_dict and dst_key in state_dict:
                state_dict[dst_key].copy_(state_dict[src_key])
    ...
    def from_pretrained(cls, cfg):
        ...
        # Handle looped architecture weight mapping
        if getattr(cfg, "lm_looped", False):
            print("Mapping pretrained weights to looped architecture...")

            n_pretrained = cfg.lm_n_blocks  # Total number of pretrained layers
            n_prelude = cfg.lm_n_prelude_layers
            n_core = cfg.lm_n_core_layers
            n_coda = cfg.lm_n_coda_layers

            # Verify we have enough pretrained layers
            min_required = n_prelude + n_core + n_coda
            if n_pretrained < min_required:
                raise ValueError(
                    f"Need at least {min_required} pretrained layers for looped architecture "
                    f"({n_prelude} prelude + {n_core} core + {n_coda} coda), but model has {n_pretrained}"
                )

            # Map prelude from first N pretrained layers
            for i in range(n_prelude):
                src_prefix = f"blocks.{i}."
                dst_prefix = f"prelude_blocks.{i}."
                cls._copy_block_weights(sd, src_prefix, dst_prefix)

            # Map core from middle layers (evenly spaced)
            core_start = n_prelude
            core_end = n_pretrained - n_coda
            if n_core == 1:
                # If only one core layer, use the middle layer
                core_indices = [(core_start + core_end) // 2]
            else:
                # Evenly space core layers across the middle region
                core_spacing = (core_end - core_start - 1) / (n_core - 1)
                core_indices = [int(core_start + i * core_spacing) for i in range(n_core)]
            ...
            # Map coda from last M pretrained layers
            ...
            print(f"Mapped {n_pretrained} pretrained layers to looped architecture:")
            print(f"  Prelude: layers 0-{n_prelude - 1}")
            print(f"  Core: layers {core_indices} (looped {cfg.lm_n_loops}x)")
            print(f"  Coda: layers {n_pretrained - n_coda}-{n_pretrained - 1}")
            print(
                f"  Effective depth: {cfg.lm_n_prelude_layers + cfg.lm_n_core_layers * cfg.lm_n_loops + cfg.lm_n_coda_layers}"
            )
