...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_path", type=str, default="assets/image.png", help="Path to the input image."
    )
    parser.add_argument(
        "--prompt", type=str, default="What is in this image?", help="Prompt for the VLM."
    )
    parser.add_argument(
        "--max_new_tokens", type=int, default=50, help="Number of new tokens to generate."
    )
    parser.add_argument(
        "--num_runs", type=int, default=10, help="Number of times to run the benchmark."
    )
    parser.add_argument(
        "--warmup_runs", type=int, default=3, help="Number of warmup runs before benchmarking."
    )
    ...
    try:
        with open(results_file) as f:
            ...
            for r in existing_results_list:
                ...
                if all(
                    k in r for k in ["vit_model_type", "mp_pixel_shuffle_factor", "lm_model_type"]
                ):
    ...
    all_combinations = list(
        itertools.product(args.vit_model_types, args.mp_pixel_shuffle_factors, args.lm_model_types)
    )
